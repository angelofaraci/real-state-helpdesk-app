...
`require_super_admin` (organizations), `require_org_admin` (users CRUD,
bridges the authenticated `User` into an `OrgScope`), and `require_org_staff`
(properties/contracts read access, admin+agent).
"""
...
from app.api.deps import require_org_admin, require_org_staff, require_super_admin
...
@pytest.mark.asyncio
@pytest.mark.parametrize("role", [UserRole.ADMIN, UserRole.AGENT])
async def test_require_org_staff_returns_an_org_scope_for_admin_or_agent(role) -> None:
    org_id = uuid4()
    principal = _user(organization_id=org_id, role=role)

    scope = await require_org_staff(principal)
...
@pytest.mark.parametrize(
    "overrides",
    [
        {"role": UserRole.TENANT},  # non-staff role
        {"role": UserRole.OWNER},  # non-staff role
        {"organization_id": None, "role": UserRole.ADMIN},  # super-admin, no org
    ],
)
async def test_require_org_staff_rejects_non_staff(overrides: dict) -> None:
    with pytest.raises(HTTPException) as exc_info:
        await require_org_staff(_user(**overrides))
